...
class HealthcareChatPage extends StatefulWidget {
  ...
  final int? sessionId; // Added session support
...
class _HealthcareChatPageState extends State<HealthcareChatPage> with TickerProviderStateMixin {
  ...
  void _sendMessage() async {
    ...
    try {
      ...
      if (_sessionId == null) {
        _sessionId = await ChatService.createSession(text.length > 30 ? '${text.substring(0, 27)}...' : text);
      }
      
      if (_sessionId != null) {
        ChatService.saveMessage(_sessionId!, userMsg);
      }
      ...
      if (mounted) {
        ...
        if (_sessionId != null) {
          ChatService.saveMessage(_sessionId!, assistantMsg);
        }
